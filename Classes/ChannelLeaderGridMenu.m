...
#import "ChannelLeaderGridMenu.h"
...
@implementation ChannelLeaderGrideMenuItem

+ (instancetype)emptyItem {
    static ChannelLeaderGrideMenuItem *emptyItem = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        emptyItem = [[ChannelLeaderGrideMenuItem alloc] initWithImage:nil title:nil action:nil];
    });
...
- (BOOL)isEqual:(id)object {
    if (![object isKindOfClass:[ChannelLeaderGrideMenuItem class]]) {
        return NO;
    }
...
@interface ChannelLeaderGridMenu ()
...
static ChannelLeaderGridMenu *rn_visibleGridMenu;

@implementation ChannelLeaderGridMenu
...
- (instancetype)initWithItems:(NSArray *)items {
    if ((self = [super init])) {
        ...
        _backgroundColor = [UIColor colorWithWhite:0 alpha:0];
        _bounces = YES;

        BOOL hasImages = [items filteredArrayUsingPredicate:[NSPredicate predicateWithBlock:^BOOL(ChannelLeaderGrideMenuItem *item, NSDictionary *bindings) {
...
- (instancetype)initWithImages:(NSArray *)images {
    ...
    for (UIImage *image in images) {
        ChannelLeaderGrideMenuItem *item = [[ChannelLeaderGrideMenuItem alloc] initWithImage:image];
...
- (instancetype)initWithTitles:(NSArray *)titles {
    ...
    for (NSString *title in titles) {
        ChannelLeaderGrideMenuItem *item = [[ChannelLeaderGrideMenuItem alloc] initWithTitle:title];
        [items addObject:item];
    }
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (self.selectedItemView != nil) {
        ChannelLeaderGrideMenuItem *item = self.items[self.selectedItemView.itemIndex];
...
- (void)setupItemViews {
    ...
    [self.items enumerateObjectsUsingBlock:^(ChannelLeaderGrideMenuItem *item, NSUInteger idx, BOOL *stop) {
...
- (void)selectItemViewAtPoint:(CGPoint)point {
    ...
    ChannelLeaderGrideMenuItem *item = self.items[selectedItemView.itemIndex];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_touchesDidMove) {
        ChannelLeaderGridMenu *menu = [ChannelLeaderGridMenu visibleGridMenu];
