...
@implementation HomePageHorizontalList
...
- (id)initWithFrame:(CGRect)frame title:(NSString *)title items:(NSMutableArray *)items
{
    ...
    if (self) {
        ...
        for(HomePageListItem *item in items) {
        ...
        dropshadowLayer.locations = [NSArray arrayWithObjects:
         ...
         dropshadowLayer.colors = [NSArray arrayWithObjects:
                                   (id)[[UIColor colorWithWhite:224.0/256.0 alpha:0.0] CGColor],
                                   (id)[[UIColor colorWithWhite:235.0/256.0 alpha:0.0] CGColor], nil];
...
- (void)itemTapped:(UITapGestureRecognizer *)recognizer {
    HomePageListItem *item = (HomePageListItem *)recognizer.view;
    NSLog(@"select");
    if (item != nil) {
